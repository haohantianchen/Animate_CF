...
from .unet_blocks_cf_atten import (
...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        # 增加额外参数use_cf_attention
        unet_use_cf_attention    = False,
    ):
        super().__init__()
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            down_block = get_down_block(
                ...
                # 增加额外参数use_cf_attention
                unet_use_cf_attention=unet_use_cf_attention,
            )
            # 将配置好的down_block添加到 self.down_blocks 列表中，这个列表存储模型中所有的down_block
    ...
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None, unet_additional_kwargs=None):
        ...
        # 新增attn_cf.的训练参数统计
        params = [p.numel() if "attn_cf." in n else 0 for n, p in model.named_parameters()]
        print(f"### Cross-Frame Attention Module Parameters: {sum(params) / 1e6} M")
